...
void writeStudentsToFile(const char *filename, student *students) {
    ...
    fprintf(file, "%s %s %s %d %d %d %d\n",
            students->surname, students->name, students->patronymic,
            students->group, students->mathematics, students->physics,
            students->chemistry);
    fclose(file);
}
...
void* inputStudents(void** head, void* arg2) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        writeStudentsToFile("students.txt", &newStudent);
    }
}
int main() {
    int size = 0;
